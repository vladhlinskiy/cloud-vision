...
public class ImagePropertiesAnnotationsToRecordTransformerTest extends BaseAnnotationsToRecordTransformerTest {
  ...
  public void testTransformSingleField() {
    ...
    Schema textAnnotationSingleFieldSchema = Schema.recordOf(
      ...
      Schema.Field.of(ImageExtractorConstants.ColorInfo.SCORE_FIELD_NAME, Schema.of(Schema.Type.FLOAT)));
    Schema schema = Schema.recordOf("transformed-record-schema",
      Schema.Field.of("path", Schema.of(Schema.Type.STRING)),
    ...
    for (int i = 0; i < DOMINANT_COLORS_ANNOTATION.getColorsList().size(); i++) {
      ...
      Assert.assertEquals(expectedColorInfo.getScore(),
        actualColorInfo.<Float>get(ImageExtractorConstants.ColorInfo.SCORE_FIELD_NAME),
        DELTA);
    }
  }
  ...
  private void assertColorEquals(ColorInfo expected, StructuredRecord actual) {
    ...
    Assert.assertEquals(expected.getScore(),
      actual.<Float>get(ImageExtractorConstants.ColorInfo.SCORE_FIELD_NAME),
      DELTA);
    Assert.assertEquals(expected.getPixelFraction(),
      actual.<Float>get(ImageExtractorConstants.ColorInfo.PIXEL_FRACTION_FIELD_NAME),
      DELTA);
    Assert.assertEquals(expected.getColor().getAlpha().getValue(),
      actual.<Float>get(ImageExtractorConstants.ColorInfo.ALPHA_FIELD_NAME),
      DELTA);
    Assert.assertEquals(expected.getColor().getRed(),
      actual.<Float>get(ImageExtractorConstants.ColorInfo.RED_FIELD_NAME),
      DELTA);
    Assert.assertEquals(expected.getColor().getGreen(),
      actual.<Float>get(ImageExtractorConstants.ColorInfo.GREEN_FIELD_NAME),
      DELTA);
    Assert.assertEquals(expected.getColor().getBlue(),
      actual.<Float>get(ImageExtractorConstants.ColorInfo.BLUE_FIELD_NAME),
      DELTA);
  }
}

...
public class SafeSearchAnnotationsToRecordTransformerTest extends BaseAnnotationsToRecordTransformerTest {
  ...
  public void testTransformSingleField() {
    ...
    Schema singleFieldSchema = Schema.recordOf("single-field", Schema.Field.of(
      ImageExtractorConstants.SafeSearchAnnotation.VIOLENCE_FIELD_NAME, Schema.of(Schema.Type.STRING)));
    Schema schema = Schema.recordOf("transformed-record-schema",
      Schema.Field.of("path", Schema.of(Schema.Type.STRING)),
    ...
    Assert.assertEquals(SAFE_SEARCH_ANNOTATION.getViolence().name(),
      actual.get(ImageExtractorConstants.SafeSearchAnnotation.VIOLENCE_FIELD_NAME));
  }
  ...
  private void assertAnnotationEquals(SafeSearchAnnotation expected, StructuredRecord actual) {
    ...
    Assert.assertEquals(adult.name(), actual.get(ImageExtractorConstants.SafeSearchAnnotation.ADULT_FIELD_NAME));

    Likelihood violence = expected.getViolence();
    Assert.assertEquals(violence.name(), actual.get(ImageExtractorConstants.SafeSearchAnnotation.VIOLENCE_FIELD_NAME));

    Likelihood spoof = expected.getSpoof();
    Assert.assertEquals(spoof.name(), actual.get(ImageExtractorConstants.SafeSearchAnnotation.SPOOF_FIELD_NAME));

    Likelihood medical = expected.getMedical();
    Assert.assertEquals(medical.name(), actual.get(ImageExtractorConstants.SafeSearchAnnotation.MEDICAL_FIELD_NAME));

    Likelihood racy = expected.getRacy();
    Assert.assertEquals(racy.name(), actual.get(ImageExtractorConstants.SafeSearchAnnotation.RACY_FIELD_NAME));
  }
}

...
public class LabelAnnotationsToRecordTransformerTest extends BaseAnnotationsToRecordTransformerTest {
  ...
  private void assertLocationsEqual(List<LocationInfo> expected, List<StructuredRecord> actual) {
    ...
    for (int i = 0; i < expected.size(); i++) {
      ...
      Assert.assertEquals(locationInfo.getLatLng().getLatitude(),
        actualLocation.<Double>get(ImageExtractorConstants.LocationInfo.LATITUDE_FIELD_NAME),
        DELTA);
      Assert.assertEquals(locationInfo.getLatLng().getLongitude(),
        actualLocation.<Double>get(ImageExtractorConstants.LocationInfo.LONGITUDE_FIELD_NAME),
        DELTA);
    }
  }
  ...
  private void assertPropertiesEqual(List<Property> expected, List<StructuredRecord> actual) {
    ...
    for (int i = 0; i < expected.size(); i++) {
      ...
      Assert.assertEquals(property.getName(), actualProperty.get(ImageExtractorConstants.Property.NAME_FIELD_NAME));
      Assert.assertEquals(property.getValue(), actualProperty.get(ImageExtractorConstants.Property.VALUE_FIELD_NAME));
      Assert.assertEquals(property.getUint64Value(),
        (long) actualProperty.get(ImageExtractorConstants.Property.UINT_64_VALUE_FIELD_NAME));
    }
  }
}
